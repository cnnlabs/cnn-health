...
class GraphiteSpikeCheck extends Check {
    ...
    generateUrl(numerator, divisor, period) {
        const urlBase = util.format('%s%s', this.graphiteBaseUrl, `from=-${period}&format=json&target=`);
        if (divisor) {
            urlBase = util.format('%s%s', urlBase, `divideSeries(summarize(sumSeries(${numerator}),"${period}","sum",true),summarize(sumSeries(${divisor}),"${period}","sum",true))`);
            return urlBase;
        } else {
            urlBase = util.format('%s%s', urlBase, `summarize(sumSeries(${numerator}),"${period}","sum",true)`);
            return urlBase;
        }
    }
    ...
    tick() {
        ...
        .catch((err) => {
            ...
            this.checkOutput = util.format('Graphite spike check failed to fetch data: %s', err.message);
        });
    }
